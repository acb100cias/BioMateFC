...
to mueve
  encontrar-vecinos
  if any? vecinos[encontrar-vecino-cercano
    ifelse distance vecino-cercano < separacion [separar][alinear cohesion]
  ]
...
to encontrar-vecino-cercano
  set vecino-cercano min-one-of vecinos [distance myself]
end

to-report promedio-direccion-vecinos
  let x sum [dx] of vecinos
  let y sum [dy] of vecinos
  ifelse x = 0 and y = 0 [report heading][report atan x y]
end

to-report promedio-direccion-movimiento
  let x mean [sin (towards myself + 180)] of vecinos
  let y mean [cos (towards myself + 180)] of vecinos
  ifelse x = 0 and y = 0 [report heading][report atan x y]
end

to vuelta-hacia [nuevo max-an]
  vuelta(subtract-headings nuevo heading) max-an
end

to vuelta-desde [nuevo max-an]
  vuelta (subtract-headings heading nuevo) max-an  
end


to vuelta [v max-an]
  ifelse (abs v) > max-an [ifelse v > 0[rt max-an][lt max-an]][rt v]
end

to separar
  vuelta-desde ([heading] of vecino-cercano) anSeparacion
end 

to alinear
  vuelta-hacia promedio-direccion-vecinos anAlineacion  
end

to cohesion
  vuelta-hacia promedio-direccion-movimiento anCohesion
  ...
  <widgets>
    <view x="368" wrappingAllowedX="true" y="10" frameRate="30" minPycor="-30" height="619" showTickCounter="true" patchSize="10" fontSize="10" wrappingAllowedY="true" width="618" tickCounterLabel="ticks" maxPycor="30" updateMode="0" maxPxcor="30" minPxcor="-30"></view>
    <button x="3" y="105" height="50" disableUntilTicks="false" forever="false" kind="Observer" width="120">setup</button>
    <button x="150" y="100" height="50" disableUntilTicks="false" forever="true" kind="Observer" width="120">go</button>
    <slider x="19" step="1" y="43" max="300" width="250" display="N" height="50" min="0" direction="Horizontal" default="50" variable="n"></slider>
    <slider x="10" step="1" y="175" max="10" width="250" display="vision" height="50" min="0" direction="Horizontal" default="5" variable="vision" units="parches"></slider>
    <slider x="15" step="1" y="240" max="6" width="250" display="separacion" height="50" min="0" direction="Horizontal" default="1" variable="separacion" units="parches"></slider>
    <slider x="10" step="1" y="300" max="45" width="250" display="anAlineacion" height="50" min="0" direction="Horizontal" default="3" variable="analineacion" units="grados"></slider>
    <slider x="15" step="1" y="355" max="45" width="250" display="ancohesion" height="50" min="0" direction="Horizontal" default="3" variable="ancohesion" units="grados"></slider>
    <slider x="25" step="0.1" y="435" max="45" width="250" display="anSeparacion" height="50" min="0" direction="Horizontal" default="1.5" variable="anseparacion" units="grados"></slider>
  </widgets>
  <info>## WHAT IS IT?
