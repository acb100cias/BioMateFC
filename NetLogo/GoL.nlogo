...
to setup
  ...
  ask patches [
    ...
    set pcolor red - 3 ;; dark blue cells are dead
    if random 100 < P [
      set pcolor pink ;; green cells are alive
    ]
  ]
  reset-ticks
end
...
to go
  ask patches [
    ...
    ifelse Moore [set live-neighbors count neighbors with [ pcolor = pink ]]
    [set live-neighbors count neighbors4 with [ pcolor = pink ]]
  ]

  ask patches [
    ;; patches with 3 green neighbors, turn (or stay) green
    if live-neighbors = 3 [ set pcolor pink ]
