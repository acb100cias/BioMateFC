...
to setup
  ...
  create-turtles N [set color red setxy random-pxcor random-pycor set size 0.3 set shape "circle" pen-down];
...
to go
  ...
  [
    ...
    if [exact_color] of p > exact_color  [ face p rt random Q fd random-float 1]
  ]
  tick
end
...
to color_code_patches
  ...
  ask patches [
    ...
    ifelse (dist <= 0.02) [ set blueness 250 ]
...
to color_patches
  diffuse chem 0.5
  ask patches [ set pcolor blue - random-float 1]
end
@#$#@#$#@
GRAPHICS-WINDOW
311
